...
[V,D(R)] = 0
\end{equation}
for all $D(R)$ symmetries in the stabilizer group of the bond. The Wigner matrices $\wignerDl =  \bra{l'm'} e^{-\theta\hat{L}\cdot\hat{n}}\ket{lm}$ parameterizes a rotation around $\hat{n}$ by $\theta$ degrees via the angular momentum opreator $\hat{L}$. % Lax p 43 Eq 2.5.18
