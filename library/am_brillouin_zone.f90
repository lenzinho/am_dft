module am_brillouin_zone
    ...
    type, public :: am_class_bz
        ...
        real(dp), allocatable :: kpt_recfrac(:,:) ! kpt(3,nkpts) kpoint - notice that vasp IBZKPT is in (reciprocal) fractional units.
...
contains
    ...
    subroutine     create_brillouin_zone(bz,kpt_recfrac,kpt_cart,bas,recbas,prec)
        !
        ! kpt_recfrac points are shifted to be between [0,1)
        ! kpt_cart           are shifted to be in the wigner-seitz bz
        !
        implicit none
        !
        class(am_class_bz), intent(out) :: bz
        real(dp), intent(in), optional :: kpt_recfrac(:,:)
        ...
        if (present(kpt_recfrac)) then
            !
            bz%nkpts = size(kpt_recfrac,2)
            allocate(bz%kpt_cart(3,bz%nkpts))
            allocate(bz%kpt_recfrac(3,bz%nkpts))
            !
            bz%kpt_recfrac = kpt_recfrac
            bz%kpt_cart    = matmul(recbas,kpt_recfrac)  
            !
        elseif (present(kpt_cart)) then
            ...
            allocate(bz%kpt_recfrac(3,bz%nkpts))
            !
            bz%kpt_recfrac = matmul(bas,kpt_cart)
            bz%kpt_cart    = kpt_cart
        else
            stop 'Either kpt_cart or kpt_recfrac must be present'
        endif
        ...
        enddo
        ! make sure kpt_recfrac is betwen [0,1)
        bz%kpt_recfrac = modulo(bz%kpt_recfrac+prec,1.0_dp)-prec
        !
        contains
    ...
    subroutine     load(bz,uc,pg,opts,flags)
        ...
        !
        call bz%create_brillouin_zone(kpt_recfrac=kpt, recbas=uc%recbas, bas=uc%bas, prec=opts%prec)
        !
...
!     end subroutine
    ...
    subroutine     get_fbz(fbz,uc,n,s,opts)
        ...
        if (opts%verbosity.ge.1) then
            ...
            call am_print_two_matrices_side_by_side(name='kpoints',&
                Atitle='fractional',A=transpose(fbz%kpt_recfrac),&
    ...
    subroutine     get_ibz(ibz,fbz,pg,opts)
        ...
        allocate(ibz%kpt_recfrac, source=fbz%kpt_recfrac(:,ind(1:k)))
        ...
        if (opts%verbosity.ge.1) then
            ...
            call am_print_two_matrices_side_by_side(name='IBZ k-points',&
                Atitle='fractional',A=transpose(ibz%kpt_recfrac),&
