        ...
        do while ( i < narg )
            ...
            select case(argument)
                case('-h')
                    ...
                    write(*,'(5x,a)') '     File from which dispersion is read to fit tight binding matrix elements.'
                    write(*,'(5x,a)') ''
...
!             endif
!             call file%get_keyword(keyword='skip'   ,isfound=isfound, i_value=opts%skip)
!             if (.not.isfound) then
!                 stop 'ERROR [parse_control_file]: skip is required for fitting'
!             endif
!         endif
