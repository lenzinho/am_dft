module am_vasp_io
    ...
    subroutine read_ibzkpt(nkpts,kpt,w_int,ntets,vtet,tet,wtet,iopt_filename,iopt_verbosity) 
    !>
    !> Reads IBZKPT into variables.
    !>
    !> nkpts number of kpoints
    !> kpt(3,nkpts) kpoint vectors
    !> w_int(nkpts) weights (integer)
    ...
    integer , allocatable, intent(out) :: w_int(:)          !> w_int(nkpts) weights
    ...
    if (verbosity .ge. 1) write(*,'(a)' ) 'Reading ibzkpts'
    !
    fid = 1
    ...
    !
    if (verbosity .ge. 1) write(*,*)
    if (verbosity .ge. 1) write(*,'(a)' ) 'Reading procar'
    !
    fid = 1
    open(unit=fid,file=trim(fname),status="old",action='read')
        !
    ...
    !
    if (verbosity .ge. 1) write(*,*)
    if (verbosity .ge. 1) write(*,'(a)' ) 'Reading eigenval'
    !
    fid = 1
    open(unit=fid,file=trim(fname),status="old",action='read')
        !
    ...
    !
    if (verbosity .ge. 1) write(*,*)
    if (verbosity .ge. 1) write(*,'(a)') 'Reading poscar'
    !
    fid = 1
    open(unit=fid,file=trim(fname),status="old",action='read')
        !
